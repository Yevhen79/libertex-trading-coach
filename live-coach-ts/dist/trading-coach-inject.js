...
(() => {
  ...
  var magW = (p) => p < 0.5 ? "почти незаметный" : p < 1.5 ? "небольшой" : p < 4 ? "заметный" : p < 8 ? "существенный" : "крупный";
  var magWW = (p) => p < 0.5 ? "скромный" : p < 1.5 ? "неплохой" : p < 4 ? "хороший" : p < 8 ? "солидный" : "крупный";
  ...
  function detect(t, all, list) {
    ...
    const lastLoss = all.filter((x) => x.closeTime <= t.startTime && pnl(x) < 0).sort((a, b) => a.closeTime - b.closeTime).pop();
    ...
    const d = (t.closeTime - t.startTime) / 6e4;
  ...
  var MAX_PATTERNS = 2;
  function buildComment(t, all, list) {
    ...
    const head = p === 0 ? `Сделка по <b>${t.alias}</b> закрылась в ноль.` : rotate(p < 0 ? LOSS : WIN, p < 0 ? "l" : "w").replace("{a}", t.alias).replace("{v}", sgn(p));
    const pctS = pct > 0 ? pct.toFixed(2) === "0.00" ? "0.01" : pct.toFixed(2) : "0.00";
    const balS = p < 0 ? ` Это <b>${magW(pct)}</b> убыток — <b style="color:${C.neg}">−${pctS}%</b> депозита ($${fk(bal)}).` : p > 0 ? ` ${capitalize(magWW(pct))} плюс — <b style="color:${C.pos}">${pctS}%</b> депозита ($${fk(bal)}).` : "";
    const moveS = t.mult >= MOVE_LEVERAGE_MIN ? ` ⚙️ Цена прошла всего <b>${move >= 0 ? "+" : ""}${move.toFixed(2)}%</b>, но плечо <b>×${t.mult}</b> превратило это в <b style="color:${p < 0 ? C.neg : C.pos}">${pin >= 0 ? "+" : ""}${pin.toFixed(1)}%</b> от вложенного (объём ~$${fk(notion)}).` : "";
    const patsArr = detect(t, all, list).slice(0, MAX_PATTERNS);
    const nudge = p < 0 && t.stopLossPrice == null ? rotate(
      [
      ...
      "nsl"
    ) : p > 0 && t.stopLossPrice != null ? rotate(
      [
  ...
  function buildReview(list) {
    ...
    const durs = list.map((t) => (t.closeTime - t.startTime) / 6e4);
    ...
    const revenge = list.filter((t) => {
      ...
      return ll && t.startTime - ll.closeTime <= REVENGE_WINDOW_MS2;
    }).length;
    let mls = 0, cw = 0, cl = 0;
    list.forEach((t) => {
      const p = pnl(t);
    ...
    const levDesc = mAvg > 500 ? `с экстремальным плечом (в среднем ×${mAvg})` : mAvg > 150 ? `с очень высоким плечом (в среднем ×${mAvg})` : mAvg > 50 ? `с высоким плечом (в среднем ×${mAvg})` : mAvg > 10 ? `с умеренным плечом (в среднем ×${mAvg})` : `с невысоким плечом (в среднем ×${mAvg})`;
    ...
    const sections = [
      ...
      },
      {
        ...
        html: gf.length ? `Пара вещей, которые мягко подмечу: ${gf.join("; ")}. Ничего страшного — просто чтобы ты это видел.` : "Явных эмоциональных всплесков не вижу — ни отыгрышей, ни резких раздуваний объёма. Ты держишь холодную голову 🕊 красиво."
      },
      {
        h: "6. Прогресс и вывод",
        html: (slp < 50 ? "Если и есть что подтянуть — это <b>дисциплина по стопам</b>, и, честно, одно это изменило бы многое. " : "Дисциплина по стопам у тебя на уровне — так держать. ") + (mAvg >= 100 ? "А если чуть снизить плечо, у маржи будет больше воздуха на обычных колебаниях." : "Плечо в разумных пределах — это бережёт твой счёт.")
      }
    ];
    const scores = [
    ...
    const habit = slp < 50 ? "ставить стоп-лосс на каждую сделку (при высоком плече — обязательно)" : mAvg >= 100 ? "снизить плечо — оно кратно усиливает риск слить маржу" : rr > 3 ? "резать убытки быстрее и давать прибыли расти" : "держать размер и плечо в разумных пределах";
  ...
  function render() {
    ...
    if (up) up.onclick = () => {
      c.vote = "up";
      render();
      toast("Спасибо за отзыв! 👍", C.pos);
    };
    if (dn) dn.onclick = () => {
      c.vote = "down";
      render();
      toast("Понял, спасибо 👎", C.t2);
    };
  }
  ...
  function showReview(r) {
    ...
    stars.forEach((star, i) => {
      ...
      star.onclick = () => {
        r.rating = i + 1;
        paint(i);
        starMsg.textContent = "Спасибо за оценку! 🙏";
      };
    });
  ...
  function processTrades(list) {
    ...
    toast(
      (S.newCount % REVIEW_EVERY === 0 ? `🧠 Готов AI Trading Review ${REVIEW_EVERY} сделок! • ` : "") + `Сделка ${sgn(lp)} — ${last.alias}`,
      lp >= 0 ? C.pos : C.neg
    );
  }
  function poll() {
    ...
    fetch(API, { headers: { Accept: "application/json" }, credentials: "include" }).then((r) => r.json()).then((j) => {
      ...
      if (!S.init) {
        ...
        S.medDur = median(list.map((t) => (t.closeTime - t.startTime) / 6e4));
        render();
      } else {
        processTrades(list);
      }

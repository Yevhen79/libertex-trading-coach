...
(() => {
    ...
    const API = "/spa/report/closed-positions?page=1&pageSize=100&order=CloseTime&orderDir=desc&searchPhrase=";
    ...
    w.__lbxCoach = S;
    const pnl = (t) => Math.round((t.equityInv - t.sumInv) * 100) / 100;
    const isWin = (t) => pnl(t) > 0;
    const isLoss = (t) => pnl(t) < 0;
    const sum = (a) => a.reduce((s, v) => s + v, 0);
    const fmt = (n) => (Math.round(Math.abs(n) * 100) / 100).toLocaleString("ru-RU", { maximumFractionDigits: 2 });
    const sgn = (n) => (n >= 0 ? "+" : "−") + "$" + fmt(n);
    const fk = (n) => n >= 1000 ? (Math.round(n / 100) / 10).toLocaleString("ru-RU") + "k" : fmt(n);
    const median = (a) => {
        if (!a.length)
            return 0;
    ...
    const magW = (p) => p < 0.5 ? "почти незаметный" : p < 1.5 ? "небольшой" : p < 4 ? "заметный" : p < 8 ? "существенный" : "крупный";
    const magWW = (p) => p < 0.5 ? "скромный" : p < 1.5 ? "неплохой" : p < 4 ? "хороший" : p < 8 ? "солидный" : "крупный";
    ...
    function comment(t, all, list) {
        ...
        const nudge = p < 0 && t.stopLossPrice == null
            ? rotate([
        ...
        if (moveS)
            blocks.push(`<div style="margin-top:11px;padding-top:11px;border-top:1px solid ${C.line};line-height:1.55">${moveS.replace(/^\s+/, "")}</div>`);
        if (patsArr.length)
            blocks.push(`<div style="margin-top:11px;padding-top:11px;border-top:1px solid ${C.line}">` +
                patsArr
                    .map((x, i) => `<div style="display:flex;gap:8px;line-height:1.5${i ? ";margin-top:6px" : ""}"><span style="color:${C.br};font-weight:800">•</span><span>${x}</span></div>`)
                    .join("") +
                `</div>`);
        if (nudge)
            blocks.push(`<div style="margin-top:11px;background:rgba(255,164,8,.09);border:1px solid rgba(255,164,8,.30);border-radius:10px;padding:9px 11px;line-height:1.5">💡 ${nudge}</div>`);
        return {
            m: mood,
            a: acc,
            ti,
    ...
    function review(list) {
        ...
        const levDesc = mAvg > 500 ? `с экстремальным плечом (в среднем ×${mAvg})`
        ...
        const gf = [];
        if (rr > 3)
            gf.push(`прибыль фиксируется рано, а убыткам даёшь течь — средний убыток в <b>${Math.round(rr)}×</b> больше среднего профита (классический перекос)`);
        if (revenge > 0)
            gf.push(`<b>${revenge}</b> сделок открыты вскоре после убытка — возможные эмоциональные отыгрыши`);
        if (mls >= 3)
            gf.push(`была серия из <b>${mls}</b> убытков подряд — момент, где важно не повышать ставки`);
        if (over > 0)
            gf.push(`<b>${over}</b> сделок с маржой заметно крупнее обычного`);
        const sections = [
            {
                h: "1. Профиль стиля",
    ...
    const elCard = box.querySelector("#lbxCard");
    const elCnt = box.querySelector("#lbxCnt");
    const elPos = box.querySelector("#lbxPos");
    function render() {
        if (!S.cards.length) {
        ...
        if (rv)
            rv.onclick = () => { if (c.review)
                showReview(c.review); };
        const up = box.querySelector("#lbxUp");
        const dn = box.querySelector("#lbxDn");
        if (up)
            up.onclick = () => { c.vote = "up"; render(); toast("Спасибо за отзыв! 👍", C.pos); };
        if (dn)
            dn.onclick = () => { c.vote = "down"; render(); toast("Понял, спасибо 👎", C.t2); };
    }
    ...
    function process(list) {
        ...
        fresh.forEach((t) => {
            ...
            if (S.newCount % REVIEW_EVERY === 0)
                card.review = review(S.newTrades.slice(-REVIEW_EVERY));
            else
                card.left = REVIEW_EVERY - (S.newCount % REVIEW_EVERY);
            S.cards.push(card);
            S.idx = S.cards.length - 1;
        });
        ...
        if (pip)
            pip.textContent = String(S.newCount);
        render();
        const last = fresh[fresh.length - 1], lp = pnl(last);
        toast((S.newCount % REVIEW_EVERY === 0 ? `🧠 Готов AI Trading Review ${REVIEW_EVERY} сделок! • ` : "") +
            `Сделка ${sgn(lp)} — ${last.alias}`, lp >= 0 ? C.pos : C.neg);
    }
    function poll() {
        readBalance();
    ...
    box.querySelector("#lbxPrev").onclick = () => { S.idx--; render(); };
    box.querySelector("#lbxNext").onclick = () => { S.idx++; render(); };
    ...
    function outsideClick(e) {
        ...
        if (box.contains(e.target) || pill.contains(e.target))
            return;
        if (document.getElementById("lbxOverlay"))
            return;
        collapse();
    }
